...
import {
  ...
  Plus, ChevronRight,
  BookOpen, Trophy, PenLine, Bell,
} from 'lucide-react'

/**
 * Corpo do dashboard de KPIs, compartilhado entre o app principal
 * (/app/dashboard, drilldown=true) e o produto Campos Pillar (/pillar, leitura).
 *
 * drilldown=false: os cards viram leitura pura — sem navegação pras telas do
 * app (docs, trilhas, chat…), que não existem no produto enxuto.
 */
export default function DashboardKPIs({ drilldown = true }: { drilldown?: boolean }) {
  ...
  const [, setMyProgress] = useState<any[]>([])
  const [loading, setLoading] = useState(true)

  // Só navega quando o modo permite (drilldown). No produto enxuto vira no-op.
  const go = (href: string) => { if (drilldown) router.push(href) }
      ...
      {(stats.alerts_unread > 0 || stats.reads_pending > 0 || stats.signs_pending > 0) && (
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-6">
          {stats.alerts_unread > 0 && (
            <button onClick={() => go('/app/alertas')} disabled={!drilldown}
              className={`flex items-center gap-3 bg-amber-500/10 border border-amber-500/30 rounded-xl p-4 text-left transition ${drilldown ? 'hover:bg-amber-500/15' : 'cursor-default'}`}>
              ...
              <div>
                ...
                <p className="text-xs text-slate-500">{drilldown ? 'Clique para ver' : 'Pendente'}</p>
              </div>
            </button>
          )}
          {stats.reads_pending > 0 && (
            <button onClick={() => go('/app/docs')} disabled={!drilldown}
              className={`flex items-center gap-3 bg-blue-500/10 border border-blue-500/30 rounded-xl p-4 text-left transition ${drilldown ? 'hover:bg-blue-500/15' : 'cursor-default'}`}>
          ...
          {stats.signs_pending > 0 && (
            <button onClick={() => go('/app/docs')} disabled={!drilldown}
              className={`flex items-center gap-3 bg-purple-500/10 border border-purple-500/30 rounded-xl p-4 text-left transition ${drilldown ? 'hover:bg-purple-500/15' : 'cursor-default'}`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1 bg-slate-900 border border-slate-800 rounded-xl p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            {drilldown && (
              <button onClick={() => go('/app/training')}
          ...
          {/* Ações rápidas — só quando há navegação pra onde ir. */}
          {drilldown && (
        ...
        <div className="lg:col-span-2 bg-slate-900 border border-slate-800 rounded-xl p-5">
          ...
          {recentDocs.length === 0 ? (
            <div className="text-center py-8">
              ...
              {drilldown && user?.role !== 'member' && (
                <button onClick={() => go('/app/docs/new')}
            ...
            <div className="space-y-2">
              {recentDocs.map(doc => (
                <button key={doc.id} onClick={() => go(`/app/docs/${doc.id}`)} disabled={!drilldown}
                  className={`w-full flex items-center gap-3 bg-slate-800/50 border border-slate-800 rounded-xl p-3.5 text-left transition group ${drilldown ? 'hover:bg-slate-800 hover:border-slate-700' : 'cursor-default'}`}>
                  <div className="w-8 h-8 rounded-lg bg-slate-700 flex items-center justify-center shrink-0 text-sm">
                    📄
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className={`text-sm font-medium text-white truncate transition ${drilldown ? 'group-hover:text-amber-300' : ''}`}>{doc.title}</p>
                  ...
                  {drilldown && <ChevronRight className="w-4 h-4 text-slate-600 group-hover:text-slate-400 shrink-0 transition"/>}
                </button>
              ))}
            </div>
          )}
